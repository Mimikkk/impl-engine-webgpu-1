  ...
  async _createTexture(buffer, config = {}) {
    ...
    if (container.vkFormat !== VK_FORMAT_UNDEFINED) {
      ...
      for (let levelIndex = 0; levelIndex < container.levels.length; levelIndex++) {
        pendings.push(
          createDataTexture(container, levelIndex).then(function (dataTexture) {
    ...
    switch (message.type) {
      ...
      case 'transcode':
        transcoderPending.then(() => {
          try {
            const { faces, buffers, width, height, hasAlpha, format, dfdTransferFn, dfdFlags } = transcode(
              message.buffer,
            );

            self.postMessage(
              { type: 'transcode', id: message.id, faces, width, height, hasAlpha, format, dfdTransferFn, dfdFlags },
              buffers,
            );
          } catch (error) {
            console.error(error);
