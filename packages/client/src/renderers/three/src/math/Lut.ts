...
export class Lut {
  ...
  constructor(colormap: ColorMap = 'rainbow', count: number = 32) {
    this.isLut = true;
  ...
  setColorMap(colormap: ColorMap, count: number = 32) {
  ...
  addColorMap(name: string, arrayOfColors: [number, number][]) {
...
export const ColorMapKeywords: Record<
  'rainbow' | 'cooltowarm' | 'blackbody' | 'grayscale' | string,
  [number, number][]
> = {
  rainbow: [
    [0.0, 0x0000ff],
...
export type ColorMap = keyof typeof ColorMapKeywords;
