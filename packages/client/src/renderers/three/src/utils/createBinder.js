...
export const createBinder = backend => {
	const self = {
		createLayout: bindingsCpu =>
		...
		createBindings: bindingsCpu => {
			const bindingsGpu = backend.get(bindingsCpu);
			bindingsGpu.layout = self.createLayout(bindingsCpu);
			bindingsGpu.group = self.createGroup(bindingsCpu, bindingsGpu.layout);
			bindingsGpu.bindings = bindingsCpu;
		},
		updateBinding: bindingCpu =>
			backend.device.queue.writeBuffer(backend.get(bindingCpu).buffer, 0, bindingCpu.buffer, 0),
		createGroup: (bindingsCpu, layoutGpu) =>
