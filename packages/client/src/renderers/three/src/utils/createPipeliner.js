...
export const createPipeliner = backend => {
  const findPrimitiveState = (object, geometry, material) => {
    const topology = backend.utils.findPrimitiveTopology(object, material);
    ...
    createRender: renderObject => {
      ...
      backend.get(pipeline).pipeline = backend.device.createRenderPipeline({
        vertex: Object.assign({}, backend.get(vertexProgram).module, {
          buffers: backend.attributeUtils.createShaderVertexBuffers(renderObject),
        }),
        fragment: Object.assign({}, backend.get(fragmentProgram).module, {
          targets: [
            {
              format: backend.utils.findCurrentColorFormat(context),
        ...
        depthStencil: {
          format: backend.utils.findCurrentDepthStencilFormat(context),
        ...
        multisample: {
          count: backend.utils.findSampleCount(renderObject.context),
