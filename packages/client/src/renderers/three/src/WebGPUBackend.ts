...
import { createBinder } from './utils/createBinder.js';
import { createPipeliner } from './utils/createPipeliner.js';
import { createTexturer } from './utils/createTexturer.js';
...
class WebGPUBackend extends Backend {
  constructor(parameters = {}) {
    ...
    this.utils = createUtilities(this);
    this.attributeUtils = createAttributer(this);
    this.bindingUtils = createBinder(this);
    this.pipelineUtils = createPipeliner(this);
  ...
  async getArrayBufferAsync(attribute) {
    return await this.attributeUtils.readBuffer(attribute);
  }
  ...
  needsUpdate(renderObject) {
    ...
    const sampleCount = utils.findSampleCount(renderObject.context);
    const colorSpace = utils.findCurrentColorSpace(renderObject.context);
    const colorFormat = utils.findCurrentColorFormat(renderObject.context);
    const depthStencilFormat = utils.findCurrentDepthStencilFormat(renderObject.context);
    const primitiveTopology = utils.findPrimitiveTopology(object, material);
  ...
  getCacheKey(renderObject) {
    ...
    return [
      utils.findSampleCount(renderContext),
      utils.findCurrentColorSpace(renderContext),
      utils.findCurrentColorFormat(renderContext),
      utils.findCurrentDepthStencilFormat(renderContext),
      utils.findPrimitiveTopology(object, material),
    ].join();
  }
  ...
  createRenderPipeline(renderObject) {
    this.pipelineUtils.createRender(renderObject);
  }

  createComputePipeline(computePipeline, bindings) {
    this.pipelineUtils.createCompute(computePipeline, bindings);
  }
  ...
  createIndexAttribute(attribute) {
    this.attributeUtils.create(attribute, GPUBufferUsage.INDEX | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST);
  }

  createAttribute(attribute) {
    this.attributeUtils.create(attribute, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST);
  }

  createStorageAttribute(attribute) {
    this.attributeUtils.create(
      attribute,
  ...
  updateAttribute(attribute) {
    this.attributeUtils.update(attribute);
  }

  destroyAttribute(attribute) {
    this.attributeUtils.destroy(attribute);
  }
