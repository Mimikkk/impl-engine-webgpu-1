...
`;
export const createMipMapState = ({ device }: Organizer) => {
  const sampler = device!.createSampler({ minFilter: GPUFilterMode.Linear as any });

  const pipelines: any = {};
  const module = device!.createShaderModule({ label: 'mipmap-generation', code: source });

  const self = {
    generate: (textureGpu: any, textureGpuDescriptor: any, baseArrayLayer = 0) => {
    ...
    readPipeline: (format: any) => {
      ...
      pipelines[format] = device!.createRenderPipeline({
        ...
        primitive: {
          topology: GPUPrimitiveTopology.TriangleStrip as any,
          stripIndexFormat: GPUIndexFormat.Uint32 as any,
        },
        layout: 'auto',
      });
